...
% Definition of top block
holdsAt2(top_of_stack(B, S), T) :- max_height(S,B,H,T), height(S, H, T).

% Covered
holdsAt2(covered(B), T) :- holdsAt(on(B2, B), T), time(T).

% S1 is taller than S2
holdsAt2(taller(S1,S2),T) :- height(S2,H2,T), height(S1,H1,T), lt(H2,H1).
...
#modeb(1, holdsAt2(top_of_stack(var(block), var(stack)), var(time))).
...
%#modeb(1, tallest_stack(var(stack), var(time))).
