...
public final class StacksWorld {
  ...
  public static String getLevel(String level) {
    ...
    while (target == null || wallToString(wall).equals(wallToString(target))) {
      ...
      else if (level.startsWith("baby") ) // basic colors and location
      {
        ...
        wall = randomWall(new int[]{3,4,5,6}, new int[]{4}, rcolors);
      ...
      else if (level.equals("littlehouse")) // basic colors and location
      {
        ...
        //wall = stringToWall("[[0,0,0], [0], [0,0]]");
        wall = randomWallToSort(7,7);
        //target = stringToWall("[[0], [0,0], [0,0,0]]");
        target = sortStacks(wall);
      }
      else {
  ...
  private static List<List<NumberValue>> randomWallToSort(int maxStacks, int maxHeight) {
    List<List<NumberValue>> wall = new ArrayList<>();
    int numStack = random.nextInt(maxStacks);
    ...
    for(int s = 0; s < numStack; s++) {
      int height = heights.get(s); 
      List<NumberValue> stack = new ArrayList<>();
      
      for (int h = 0; h < height; h++)
        stack.add(Color.randomColor(3));
      wall.add(stack);
    }
    return wall;
  }

  private static List<List<NumberValue>> sortStacks(List<List<NumberValue>> wall) {
    Ordering<List<NumberValue>> bySize = new Ordering<List<NumberValue>>() {
       @Override
       public int compare(List<NumberValue> s1, List<NumberValue> s2) {
          return Ints.compare(Iterables.size(s1), Iterables.size(s2));
       }
    };

    List<List<NumberValue>> sortedWall = bySize.sortedCopy(wall);
    return sortedWall;
  }
  ...
  public static List<List<NumberValue>> sortStacks(ContextValue context) {
    List<List<NumberValue>>  wall = getWallFromContext(context);
    System.out.println("IN THE SORTING FUNCTION BITCHEEES");
